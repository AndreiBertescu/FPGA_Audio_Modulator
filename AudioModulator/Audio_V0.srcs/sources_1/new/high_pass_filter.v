...
wire signed [32-1 : 0] audio_prev_in;
reg  signed [32-1 : 0] audio_prev_in_d;
...
reg signed [32-1 : 0] x_prev_left;
reg signed [32-1 : 0] x_prev_right;

reg signed [32-1 : 0] y_plus_x;
reg signed [64-1 : 0] a_mult_x_minus_xprev;
...
assign audio_prev_in  = ~tdm_counter[5] ? x_prev_left : x_prev_right;
...
always @(posedge clk_i) begin
    ...
    audio_prev_in_d  <= #TP audio_prev_in;
...
// Filter equation
always @(posedge clk_i) begin
	a_mult_x_minus_xprev <= #TP ((32'sh7FFF_FFFF + a_d) >>> 1) * (audio_in_d - audio_prev_in_d);
	a_mult_y  			 <= #TP a_d * audio_prev_out_d;
	y_plus_x  	    	 <= #TP (a_mult_y  >>> 31) + (a_mult_x_minus_xprev  >>> 31);
end
...
always @(posedge clk_i or posedge reset_i) begin
	if(reset_i) begin
		...
		x_prev_left  <= #TP left_ch_i;
		x_prev_right <= #TP right_ch_i;
	end
end
...
always @(posedge clk_i or posedge reset_i) begin
	if(reset_i) begin
		...
		left_ch_o  <= #TP ~tdm_counter[5] ? y_plus_x : left_ch_o;
		right_ch_o <= #TP  tdm_counter[5] ? y_plus_x : right_ch_o;
	end
end
